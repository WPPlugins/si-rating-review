...
Version: 1.1
Author: Hiren Patel, Kuldeep Vyas
...
if(isset($_POST['rating_submit']))
{
      ...
      $prevRatingResult = mysql_query($prevRatingQuery);
	  if(mysql_num_rows($prevRatingResult) > 0)
	  {	
		//echo "Sorry! but you have Already Voted";
	 } 
	else
	{				
        $query = "INSERT INTO $table_name(rating_postid,rating_posttitle,rating_postdesc,rating_count,rating_timestamp,rating_ip,rating_username,rating_userid,rating_url) 
        VALUES('".$postID."','".$post_title."','".$comments."','".$ratingPoints."','".$timestamp."','".$ip_address."','".$user_name."','".$user_id."','".$page_url."')";
...
?>
	<p class="btn"><a href="javascript:void(0)" id="rate-review-link">Rate and Review <?php the_title();?></a></p>						 
	...
	<?php 
	 ...
	 $prevRatingResult1 = mysql_query($prevRatingQuery1);							
	 $already=0;
		while($prevRatingRow1 = mysql_fetch_array($prevRatingResult1,MYSQL_BOTH))
		{							
			if (in_array($ip_address, explode(",",$prevRatingRow1['rating_ip'])))
			{
				//echo "Sorry! but you have already Voted.<br/>";
				$already=1;
			}							
		}							
		if($already==1) { echo "Sorry! but you have already Voted.<br/>";}
		else {?>																
						...
						<div style="visibility:hidden;height:0px;" itemscope itemtype="http://schema.org/Review">
							 <div itemprop="itemReviewed" itemscope itemtype="http://schema.org/Service">
							   <span itemprop="name"><?php echo $prevRatingRow['rating_posttitle']?></span>
							 </div>
							 <span itemprop="reviewRating" itemscope itemtype="http://schema.org/Rating">
							   <span itemprop="ratingValue"><?php echo $prevRatingRow['rating_count']?></span>
							 </span> stars -
							 <b>"<span itemprop="name"><?php echo $prevRatingRow['rating_username']?>.</span>" </b>
							 <span itemprop="author" itemscope itemtype="http://schema.org/Person">
							   <span itemprop="name"><?php echo $prevRatingRow['rating_username']?></span>
							 </span>
							 <span itemprop="reviewBody"><?php echo $prevRatingRow['rating_postdesc']?></span>
						   </div>
							<input name="rating" value="<?php echo esc_attr($prevRatingRow['rating_count']);?>" id="rating_star_<?php echo $i ?>" type="hidden" /><br/>
							<?php  echo '<div style="margin-bottom:10px;float:left;width:100%;"><span><strong>'.$i.".</strong> ".$prevRatingRow['rating_postdesc'].'</span>';?><br/>							
							<?php $i++;
							if ( current_user_can( 'administrator' ) ) {
							echo "<a class='rating_id' href='javascript:void(0);' id='".$prevRatingRow['rating_id']."'>Edit Rating and Review</a>"."<br/>";							
							}
							?>						
						...
						$edit_rating=mysql_query("SELECT * from $table_name where rating_id='".$eid."'");
						...
						{							
							...
							$update_record=mysql_query("UPDATE $table_name SET 
								rating_postdesc='$desc',
								rating_count='$rating_count' where rating_id='$current_edit_id'");
							if($update_record)
							{
								$updated=1;?>
						...
						<table>							
							<tr>
								<td>Description</td>
								<td><textarea name="description"><?php  echo esc_textarea($result['rating_postdesc']); ?></textarea></td>
							</tr>
							<tr>
								<td>Rating Count</td>
								<td><input type="text" name="rating_count" value="<?php  echo esc_attr($result['rating_count']);?>" /></td>
							</tr>							
							<tr>
								<td></td>
...
<?php function SIRR_add_shortcode_to_post_type( $content ) 
{
  	global $post;
  	$get_option=array();
  	$cur_id=get_the_ID();
