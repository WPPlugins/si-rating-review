...
<?php if(isset($_GET['delete_id']))
					{
						$did=$_GET['delete_id'];												
						$delete_rating=mysql_query("DELETE from $table_name where rating_id='".$did."'");
						if($delete_rating)
						{							
							$review=1;
						}
...
<?php if($review==1) echo "<h3 align='center' style='color:red'>Rating has been Deleted.</h3>";?>
	...
	<?php 
	$table_name=$wpdb->prefix.'si_rating_review';
	$query=mysql_query("SELECT * from $table_name order by rating_id DESC");
	$i=1;	
	...
	$query=mysql_query('SELECT option_value from wp_options where option_name="rating_author_info"');
	$result=mysql_fetch_row($query);
?>
<form name="author_info" action="" method="post">
		<label><input type="checkbox" id="disable" <?php if($result[0]==1){ ?> checked="true" <?php } ?> name="disable">
		Enable Author info </label>	
	...
	<div class="develop_by" id="first_list_bx" style="position:absolute;right:0px;padding-right:10px;">Developed By 
	<a href="http://www.satvikinfotech.com/" target="_blank" style="text-decoration:none;">Satvik Infotech</a>
	</div>
<?php  } ?>